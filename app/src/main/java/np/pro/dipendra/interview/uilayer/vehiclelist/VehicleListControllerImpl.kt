...
class VehicleListControllerImpl @Inject constructor(
    ...
    private lateinit var pagingSource: VehicleListPagedSource

    private fun getPagedSource(): VehicleListPagedSource {
        return VehicleListPagedSource(vehiclesRepository, searchText)
    }
        ...
        when (uiAction) {
            is InitialView -> {
                pagingData = Pager(config = PagingConfig(100),
                    initialKey = 1,
                    pagingSourceFactory = {
                        pagingSource = getPagedSource()
                        pagingSource
                    }).flow
            }
            is SearchText -> {
                pagingSource.invalidate()
                searchText = uiAction.text
            }
            is Retry -> retry()
            is ItemClick -> {
                openDetail(uiAction)
